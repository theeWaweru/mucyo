...
export default async function handler(req, res) {
  ...
  try {
    ...
    const infoData = JSON.parse(formData.fields.info);

    // Upload image if provided
    if (formData.files.image) {
      const blob = await put(
        `info/${infoData.id}-${Date.now()}.jpg`,
    ...
    const index = json.infoSection.findIndex((i) => i.id === infoData.id);
    if (index >= 0) {
      json.infoSection[index] = infoData;
    } else {
      json.infoSection.push(infoData);
    }

    await fs.writeFile(projectsPath, JSON.stringify(json, null, 2));

    return res.json({ success: true, info: infoData });
  } catch (error) {
